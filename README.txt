...
Changelog
---------
Version 4.14.0.3: Changed 'My name' list behaviour - this UI component use to show all opponent driver names in the app (about 10000 names) as well the specially recorded 'personalisations' (about 1000 names) which made it unreliable for some users. By default it now hides the opponent driver names. Enabling the "Allow opponent names in 'My name' list" property will add all opponent driver names to this list allowing the app to generate personalisations for them (note that these won't sound as natural as the specially recorded personalisation); AMS2 - added missing car class mappings; RF2 - fixed tyre selection issues in pit menu manager when selecting some tyre types; R3E - fixed some missing incident points messages; Various minor bug fixes; RBR - automatically install the Crew Chief plugin
